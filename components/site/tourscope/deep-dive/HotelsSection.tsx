...
import { HotelResultsSlice } from "@/components/site/tourscope/deep-dive/HotelResultsSlice";

/**
 * The deep dive's second section: hotels.
 *
 * Three movements, in the order a reader needs them — the claim, the product
 * making it, and the facts the product cannot draw.
 *
 * ── Why a ledger and not cards ──────────────────────────────────────────────
 * The flights section above ends in two walls of carrier logos, because there
 * the inventory IS the evidence — 45 marks a reader recognises. Hotels has no
 * logo set to show (a wholesaler's catalogue is a number, not a wall), so the
 * closing movement is four written facts.
 *
 * They are hairline ROWS, not a card grid, and the weight is the whole point:
 * this section already spends its visual budget on a big framed exhibit, and a
 * 2×2 of bordered cards under it would compete with that frame for the same
 * attention while saying less. The ledger is the site's quietest way to list
 * facts — the same anatomy the "more than a booking tool" list and the business
 * engine use, one step down in scale because this one is nested inside a
 * section rather than owning a page section of its own.
 *
 * ── Why the row ordinals are muted and not purple ───────────────────────────
 * The page's ledger anatomy draws its two-digit index in `ts-purple-text`. Here
 * the SECTION's own ordinal is already a purple two-digit index a few hundred
 * pixels above, and a second purple `02` inside the same section reads as the
...
export async function HotelsSection({ num }: { num: string }) {
  const t = await getTranslations("TourScope.deepDive.hotels");

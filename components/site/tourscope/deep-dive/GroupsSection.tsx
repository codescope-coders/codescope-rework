...
import { GroupToursSlice } from "@/components/site/tourscope/deep-dive/GroupToursSlice";

/**
 * The deep dive's third section: group tours.
 *
 ...
 * section above it, deliberately: by the third section the shape itself is a
 * signal that these are chapters of one reference rather than three pitches.
 *
 * ── Why a ledger and not cards ──────────────────────────────────────────────
 * The flights section ends in two walls of carrier logos, because there the
 * inventory IS the evidence — 45 marks a reader recognises. Group tours has no
 * such wall by construction: the inventory is the OPERATOR'S OWN, so there is
 * no supplier set to show and the only honest closing movement is four written
 ...
 * pixels above, and a second purple `03` inside the same section reads as the
...
export async function GroupsSection({ num }: { num: string }) {
  const t = await getTranslations("TourScope.deepDive.groups");

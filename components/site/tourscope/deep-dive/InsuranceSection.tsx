...
import { InsuranceQuotesSlice } from "@/components/site/tourscope/deep-dive/InsuranceQuotesSlice";

/**
 * The deep dive's fifth section: travel insurance.
 *
 * Three movements, in the order a reader needs them — the claim, the product
 * making it, and the facts the product cannot draw. Same anatomy as the four
 * sections above it, deliberately: by the fifth section the shape itself is the
 * signal that these are chapters of one reference rather than five pitches, and
 * a section that broke the pattern here would read as a different kind of thing
 ...
 * inventory IS the evidence — 45 marks a reader recognises. Insurance has no
 * such wall available: an operator's cover comes from ONE contracted insurer at
 * a time, so a "supplier wall" here would be a single logo, and it would be the
 * insurer's mark on a page that is not theirs. The only honest closing movement
 * is four written facts about what the vertical actually does.
 *
 ...
 * pixels above, and a second purple `05` inside the same section reads as the
...
export async function InsuranceSection({ num }: { num: string }) {
  const t = await getTranslations("TourScope.deepDive.insurance");

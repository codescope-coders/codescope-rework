...
interface Counter {
  ...
  count: number | string;
  icon?: ReactNode;
  type: "icon" | "online" | "offline" | "growth";
}
    ...
    if (data?.payload) {
      setCounters([
        ...
        },
        {
          type: "online",
          title: "Active Jobs",
          count: data?.payload?.activeJobs,
        },
        {
          type: "offline",
          title: "Closed Jobs",
          count: data?.payload?.closedJobs,
        },
      ]);
    }
  }, [data]);
    ...
    <section className="rounded-xl shadow-xs flex flex-wrap gap-3">
      {isPending &&
        counters.map((counter, i) => (
          <Skeleton
            key={i}
            className="px-5 py-3 hover:bg-primary/5 group rounded-lg border border-gray-300/80 hover:border-primary duration-200 flex-1 flex flex-col justify-between gap-3 min-w-full md:min-w-xs"
          >
